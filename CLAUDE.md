...
- **TopBar + ScoreDimensions are phase-gated** (`{isDone && …}`) — Interview phase is chrome-free; the sticky footer carries every session-level control.
...
- **Results nested nav:** `resultsStep` (0 = Overview, 1..N = per-turn); `resultsStepKey` force-remounts `<section>` on step change; `resultsDirection` picks `anim-slide-in-left` / `anim-slide-in-right`. Dots-as-tabs + Back / "Review turn N+1" / "Start another session" buttons. "Start another" → `navigate('/')`.
