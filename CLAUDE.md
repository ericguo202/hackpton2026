...
- **Industry & role autocomplete (required-selection combobox).** Onboarding + Personalize fill `industry` / `target_role` via `/api/v1/validation/industries?q=` and `/api/v1/validation/roles?q=&industry=` (returns `SuggestionsOut { suggestions: str[≤5], flagged, message }`). `app/services/profile_validation.py:suggest_industries / suggest_roles` run a three-stage pipeline (moderation MUST precede the billed LLM): (1) deterministic `_looks_like_junk` reject (empty / prompt-injection / direct-request / gibberish / overlong) → empty, no network; (2) OpenAI `check_moderation` → `flagged=True` + empty on hard-block; (3) LLM on `google/gemini-2.5-flash-lite` (primary) → `openai/gpt-oss-120b` fallback, JSON-only `{"suggestions":[...]}`, sanitized to ≤5 unique Title-Case strings. **Everything fails soft** → empty list. Suggestions are **semantic, not literal completions** (model told to include a normalized version of input as escape hatch). **Role suggestions conditioned on chosen industry** (empty → industry-agnostic). UI: shared `frontend/src/components/SuggestionCombobox.tsx` (ARIA listbox, 350ms debounce via `useDebouncedValue`) wrapped by `IndustryAutocompleteField` / `RoleAutocompleteField`. **Required selection** — only a picked suggestion advances (`selected` boolean gate); free-typed text never advances, editing re-arms it; picking in onboarding auto-advances the step. Validation `category` no longer returned (recomputed at session time by `company_research`). Gate is client-side only.
...
- **Incidents event log (internal/admin-only).** `incidents` stores a best-effort audit trail: `user_created`, `user_signed_in`, `moderation_request`, `interview_session_started`, `error`. Writes go through `app/services/incidents.py` in a fresh DB session so logging failures never break the request. Payloads capped (`sent_content` 10k, `error` 8k, large JSON 2k). Auth events are **backend-observed** (no Clerk webhooks): `user_created` on first local row creation; `user_signed_in` dedupes by Clerk `claims.sid`. Moderation incidents log only when the OpenAI API fires (not deterministic local rejects), with source metadata (`onboarding.short_bio`, `sessions.company`, `turn.transcript`); hard-blocks/failures are `warning`. `interview_session_started` logs only after session persistence. 5xx/unhandled errors and background eval failures log `error`; normal 4xx does not.
