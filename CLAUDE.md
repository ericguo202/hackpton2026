...
- **Field-tailored, confusion-aware follow-up question.** The follow-up generator (`app/services/followup.py`, Gemini 2.5 Flash) used to be a single-message text prompt that saw only `(question, transcript)` — meaning a Healthcare follow-up sounded identical to a Finance one, and an off-topic / nonsensical opening answer would yield an equally off-topic follow-up that echoed the nonsense back. Three coupled changes fix this. **(1) Brief threading.** `generate_followup` now also accepts `category`, `role_signals`, and `sample_question_themes` (the same three fields the opening-question generator consumes). `_followup_and_tts` in `sessions.py` pulls them from the already-parsed `brief_out` and passes them through; legacy sessions with `brief_out is None` still work because all three params default to `None`. The user prompt renders a `Context:` block using the **same empty-omission pattern** as `opening_question._company_digest` — empty sections are omitted entirely (never rendered as "(none)" placeholders), because the placeholder form cues the model to invent role framing from its own priors. **(2) Confused-candidate rule.** A new system-prompt rule explicitly tells the model: if the answer is off-topic / nonsensical / single-word ("test test, this is a test", "I am a very big drinker"), do NOT pretend it was substantive and do NOT quote the off-topic phrase back; instead gently redirect by re-asking the original question with a more concrete framing. **(3) Output-format rule + narrow sanitizer.** The system prompt has an explicit positive/negative contrast — a prefatory **statement** about the company or candidate is fine and often welcome (e.g. "Anthropic values AI safety. When you said you used AI tools…"); meta-reasoning **about the model's own thought process** is forbidden ("The user seems confused...", "Let me redirect by..."). The output is also run through a narrow `_sanitize_followup` that strips wrap-quotes, leading `Question:` / `Q:` / `Follow-up:` labels, and markdown asterisks. **A heuristic "drop preamble" trimmer was DELIBERATELY NOT added** — walking backward from the final `?` to a sentence boundary would false-positive on legitimate prefatory framings and strip useful company context, so meta-reasoning suppression lives in the prompt, not in post-hoc trimming. The narrow sanitizer is restricted to syntactic noise (quotes / labels / asterisks) that the follow-up never legitimately contains.
- **Two-turn interview session with auto-submit.** Each session is a locked two-turn loop: one opening question + one follow-up that references the first answer. The practice page has an **Auto-Submit** toggle (persisted per user) — on, tapping "End recording" in the sticky footer fires the turn submission the moment MediaRecorder flushes the last chunk; off, the user sees a Submit / Re-record preview rendered inside the `CameraColumn` (in the same 16:9 slot where the live camera was). Auto-submit has a one-shot retry on transient LLM errors so a flaky model call doesn't strand the session. Submit / Re-record both lock with `disabled={submitting}` while the POST is in flight so a double-click can't race the request.
...
- **Balanced structured feedback per turn.** The evaluator's scoring dimensions stay fixed, but the feedback layer is structured as `positive_moments`, `main_takeaway`, `improvement_moments`, and `quick_wins`. Positive and improvement moments must quote exact transcript snippets. Positive moments explain what worked and what to keep doing; improvement moments explain why a phrase weakened the answer and give a concrete, bite-sized suggestion, not a polished replacement answer. Legacy `coaching_moments` are still accepted as fallback for older saved turns. Every string field in `FeedbackDetail` has both a **prompt-side char budget** (told to DeepSeek per field — 120 for `transcript_snippet`, 240 for `keep_doing` / `main_takeaway`, 330 for the three prose fields) **and a Pydantic `max_length`** ~30 chars above each prompt cap (270 / 270 / 390 respectively). A `BeforeValidator` truncator clips anything past the schema cap so a single overlong field can no longer `ValidationError` the whole `EvaluatorOutput` — this is the load-bearing reason turn-1 scores stay populated even when DeepSeek occasionally over-quotes. `transcript_snippet` truncates without ellipsis so the clipped value remains a substring of the candidate transcript and survives `_drop_unanchored_moments`. Two beta-tester-driven rules round this out. **(1) Empty positives for non-answers.** A tiered rule in `feedback_detail.positive_moments` returns an empty `positive_moments` array when the answer is unintelligible (laughter, gibberish, mic-test utterances like "test test"), clearly off-topic, or inappropriate for a professional interview — no invented praise to soften feedback — with companion guidance steering `improvement_moments` to `issue_type: does_not_answer_question` / `off_track` and `main_takeaway` to plainly state the response did not address the question. The "still include one positive" clause now applies only to weak-but-genuine attempts, so real-but-imperfect answers still get encouragement. Mirrors `followup.py`'s confused-candidate rule — same failure mode, same handling. **(2) Distinct snippets across `improvement_moments`.** Two-layer defense: the prompt forbids quoting the same sentence in two moments (combine multi-weakness phrases into one moment with the most important `issue_type`); the backend pairs that with a `_dedupe_by_snippet` helper in `evaluator.py` invoked inside `_drop_unanchored_moments` after the anchoring filter — exact-string equality only (substring/overlap dedup is deliberately NOT added; that drifts into heuristic territory and risks false-positives on legitimate distinct quotes, per the `prompt_rule_vs_post_trim` memory). First-occurrence wins to preserve the model's primary judgment; applied to both moment lists for symmetry.
...
Backed by a clean auth seam (Clerk JWT verified against the Clerk JWKS), the FastAPI backend, and a small set of straight-line LLM calls routed through OpenRouter. No multi-agent loop — just four straight-line prompt calls per session (research + opening question once at start; follow-up + evaluator once per non-final turn, running in parallel).
...
**Four straight-line LLM calls per session — not a multi-agent loop. All go through OpenRouter. Calls #1 and #2 fire once at session start; #3 and #4 fire once per non-final turn (the follow-up and the evaluator run in parallel — follow-up gates the user-visible response, evaluator runs as a detached background task):**
...
3. Follow-up question (non-final turns only): `google/gemini-2.5-flash` via `app/services/followup.py`. Runs in parallel with the evaluator — uses a system+user message split. The system prompt carries the existing hard rules (10–25 words, references something concrete, ends in `?`) plus the **confused-candidate rule** (do not pretend a nonsensical answer was substantive; gently redirect) and the **output-format rule** that contrasts legitimate prefatory framing against meta-reasoning. The user prompt assembles the question + transcript with an optional `Context:` block carrying `Field: {category}`, `role_signals`, and `sample_question_themes` — rendered only when each source is non-empty, mirroring `opening_question._company_digest`. Output passes through `_sanitize_followup` (strips wrap-quotes, leading `Question:` / `Q:` / `Follow-up:` labels, markdown asterisks — deliberately narrow; no preamble-trimming heuristic).
4. Evaluate: `deepseek/deepseek-v3.2` in JSON mode (once per turn, repeated; runs as a detached background task that writes scores to the DB after the follow-up + TTS already streamed back to the user). The rubric criteria injected into the evaluator's system prompt are selected from `app/services/_field_rubrics.FIELD_RUBRICS` keyed on `brief.category` (same key that picked the opening prompt and that's now also threaded into the follow-up — guarantees one consistent field identity across opening / follow-up / scoring per session). Source-of-truth markdown lives in `backend/prompts/evaluator_prompts.md`.
...
- **Feedback detail:** `positive_moments` is capped at 3, `improvement_moments` at 4, and `quick_wins` at 3. Moment snippets are dropped server-side unless the quoted `transcript_snippet` appears exactly in the candidate transcript, then deduplicated by stripped-snippet equality (first-occurrence wins) so two `improvement_moments` can't both reach the UI quoting the same sentence with contradictory advice. `positive_moments` is empty (not "one honest moment") for unintelligible / off-topic / inappropriate answers — see the "Empty positives for non-answers" rule above. `how_to_strengthen` should be concrete and small, such as adding a customer concern, one reasoning sentence, or a small result. Do not generate a full polished answer.
...
| --------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Interview | `!isDone && currentQ` | Full-viewport `flex h-screen flex-col` shell: body grid (`<QuestionColumn>` + `<CameraColumn>` + optional `<TranscriptColumn>`) over a sticky `<PracticeFooter>` + `<QuitConfirmDialog>` |
| Results   | `isDone`              | Stepped Overview + per-turn `<ReplayCoachCard>`                                                                                                                                       |

**TopBar + ScoreDimensions are phase-gated.** Both are wrapped in `{isDone && …}` so they only mount during Results — Interview phase is deliberately chrome-free; the new sticky footer carries every session-level control instead.

**Interview body layout (`min-[900px]:grid` desktop / `flex flex-col` mobile).** Desktop `grid-template-columns` flips between `[33%_67%]` (transcript closed) and `[25%_50%_25%]` (transcript open). The camera box inside `CameraColumn` is locked to **`w-[45vw]` on desktop** regardless of which template is active — that's what guarantees the camera preview never resizes when the user toggles the transcript column; the grid columns flex around the fixed-width box. Mobile uses `w-full` and stacks question → camera → transcript vertically below a sticky footer. Vertical alignment differs by viewport (`min-[900px]:justify-center` on the columns) so desktop centers content while mobile hugs to the top so the camera doesn't sit at the bottom of a tall empty slot.
...
- `endingTurn` — latches true on the footer's "End recording" tap so the spinner shows immediately instead of flashing the preview UI for the ~tens-of-ms it takes MediaRecorder to flush its final chunk.
- `submittingTurn` — true while `/turns` is in flight. Disables the preview Submit / Re-record buttons via `<CameraColumn submitting={…} />` so a double-click can't race the request.
...
- `autoSubmit` (persisted via `useLocalStoragePref` under key `auto_submit_enabled`; the toggle lives in `Home.tsx`'s VoicePicker, Practice reads the same key) flips the whole flow: on → stop auto-submits; off → stop shows the Submit / Re-record preview inside the `CameraColumn` (replacing the live preview in the same 16:9 slot, so no layout jump).
- `replayKey` — bumped on **Re-record OR the footer's Restart-turn button** so the `<audio>` element inside `QuestionColumn` remounts and `autoPlay` re-fires; its existing `onEnded` handler then restarts the recorder. Reuses the same question without an imperative ref API.
- `showTranscript` — controls whether the transcript column (desktop) / inline section (mobile) renders. Footer button is disabled on turn 1 (`turnResults.length === 0`); the desktop column also has its own X close button.
- `showQuitConfirm` — toggles `<QuitConfirmDialog>` open/closed. The dialog's `onConfirm` calls a `handleQuit` that stops + resets the recorder and `navigate('/')`. Quitting mid-session **does not** burn a daily-limit slot (the counter increments at session finalization in the backend's `submit_turn` final-turn branch, not at creation).
