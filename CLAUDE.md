...
- **Evaluator prompt-injection hardening.** The candidate transcript is the one OpenRouter input whose output drives persisted scores, so it gets two layers. (1) **Delimiters + system clause:** `_build_prompt` wraps the current transcript AND prior-turn answers in `<candidate_answer>` tags, and `_INJECTION_SYSTEM_CLAUSE` (appended to `build_system_instruction(...)`) tells the model to treat tag contents as untrusted data and never obey instructions/score demands inside them. (2) **Deterministic initial gate:** `_INJECTION_RE.search(transcript)` runs **before** any LLM spend — a hit returns `_injection_nonanswer()` (all five content scores `0`, no positives, off-track takeaway), denying score-gaming with a zeroed turn rather than an inflated one; `delivery` is still computed from `cv_summary` (server-side, not gameable). `_INJECTION_RE` mirrors `profile_validation._PROMPT_INJECTION_RE` **minus `act as`** (common in real answers — "I had to act as the team lead" — and this gate zeroes the turn, so keeping it would false-positive genuine responses); keep the two in sync when adding new unambiguous markers. `company_research` / `profile_validation` already had delimiter + untrusted-data framing; `opening_question` / `followup` / `coaching` remain un-wrapped but self-targeting (output returns to the same user).
