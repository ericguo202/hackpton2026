...
Floating, **non-modal** career-advisor chat scoped to one turn. Backend contract + persona rules in `../CLAUDE.md` → "Ask Tutor". Components under `src/components/session-detail/ask-tutor/`; mounted by `TurnPanel.tsx` **only** (SessionDetail) — Practice mounts no `AskTutorContext` provider, so the tutor never appears there.
...
- **Desktop window mechanics (≥900px):** the window is `position: fixed` anchored bottom-LEFT (bottom-right is the global feedback FAB). Draggable by its header (translate offset via `--atx/--aty` CSS vars + a desktop-gated rule, so the mobile bottom-sheet layout is untouched); header **expand** toggle fills available height + widens via `--atw/--ath`; double-click header resets position + size; viewport-resize re-clamps. Below 900px it's a full-width bottom sheet (no drag/expand).
...
- **Types** in `src/types/tutor.ts` mirror `backend/app/schemas/tutor.py` by hand (no codegen). On `ApiError`: 422 → soft tutor bubble using the server detail (moderation refusal / redirect); 503 → "content checks unavailable" bubble.
