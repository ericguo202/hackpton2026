...
- **Voice pace toggle** (`AdvancedPanel.tsx:SpeedToggle`, under the voice grid) — "Normal"/"Slower" segmented control writing `speechSpeed` state in `Home.tsx` (default `SPEECH_SPEED_NORMAL = 1.1`; "Slower" = `SPEECH_SPEED_SLOWER = 0.9`), sent as `speech_speed` in the `POST /sessions` body. Aimed at non-native English speakers; backend maps it to ElevenLabs `voice_settings.speed` (see `../CLAUDE.md` → "Interview voices → Speech pace toggle"). Both the constants and range mirror `tts.py`.
