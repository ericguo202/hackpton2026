...
**Adding a PUBLIC route is four hand-edits — nothing auto-discovers one.** (1) `App.tsx` route + `lazyWithRetry` import; (2) `RouteSeo.tsx` `PUBLIC_ROUTE_METADATA` — being in that map is what makes the page `index, follow` with a canonical URL, and omitting it silently ships the page `noindex`; (3) `public/sitemap.xml`; (4) `vercel.json` — public routes are *absent* from its `X-Robots-Tag: noindex` source list, so adding one there would un-index it. The sitemap is load-bearing beyond SEO: `scripts/prerender.mjs` (run by `npm run build`) derives its route list from `dist/sitemap.xml`, so a public page missing from the sitemap also ships with no prerendered HTML. Today's public set: `/`, `/scoring`, `/pricing`, and the three `/legal/*` pages. Their chrome is auth-bivalent — app nav + `AccountButton` when signed in, `legalMenu` + the shared `SignInLink` when not (see `Scoring.tsx` / `Pricing.tsx`).
