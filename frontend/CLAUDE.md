...
Vite emits ONE `index.html` with an empty `<div id="root">`, so crawlers that don't run JS (Bing, Slack/LinkedIn/X unfurlers, most AI crawlers) saw no copy and the homepage's metadata on **every** URL. The build's last stage renders the six public pages to their own files: `dist/index.html`, `dist/scoring/index.html`, `dist/pricing/index.html`, `dist/legal/*/index.html`.

- **Plain Node `renderToString`, no browser.** `src/prerender/entry.tsx` mounts each page directly under `StaticRouter`, **bypassing `Root.tsx`/`App.tsx`** — that shell is client-only by construction (`ClerkProvider` + `BrowserRouter`, and `AnalyticsConsentBanner` reads `document.cookie` during render). An earlier headless-Chromium version was removed: Vercel restores its `node_modules` build cache _before_ install, so puppeteer's postinstall never re-downloaded Chrome, and the browser lived outside the (non-configurable) cached paths. Warm cache ⇒ permanently broken deploys.
- **One SSR-only alias closes the only gap.** `@clerk/react` → `src/prerender/clerk-stub.tsx`, passed **inline** to vite's `build()` and never written into `vite.config.ts`, so it cannot reach the client bundle. The blocker was a missing React _context_ (`useAuth` in `Scoring.tsx`), not a browser API. The stub answers "signed out", which is the app's genuine first frame anyway (Clerk loads async). **It answers `false` for any page added to the list, silently** — fine while those pages only vary their _chrome_ by auth.
...
- **Three files must move together when you add a public page:** `public/sitemap.xml` (the route list is _derived_ from it), `src/lib/routeMetadata.ts` (title/description/canonical), and `PRERENDER_PAGES` in `src/prerender/entry.tsx`. Miss one and **the build fails loud** naming the missing file — this used to be a silent-drift trap.
- **`lib/routeMetadata.ts` is the single source of truth**, read by `RouteSeo.tsx` at runtime _and_ by the prerenderer at build time. Keep the tag set the prerenderer injects in sync with the one `RouteSeo` writes.
- `PRERENDER_SKIP=1` degrades to **metadata-only** (head tags, no body copy) rather than skipping everything.
- The consent banner is now _absent_ from the render tree rather than suppressed inside it, so its copy can't be frozen in as a page's search-result description.
- **`vercel.json`'s catch-all rewrite `/(.*) → /index.html` must never shadow these files.** Vercel matches static files BEFORE rewrites, so `dist/scoring/index.html` wins and the catch-all only handles the JS-only routes — **verified against a real preview deploy on 2026-09-06**, after being an untested assumption through three sessions. If you ever narrow, reorder, or replace that rewrite, re-run the check, because getting it wrong **fails silently**: green build, green deploy, and every prerendered page quietly serving the homepage shell instead. The check is `curl <deploy>/robots.txt` (must be plain text, not HTML) and `curl <deploy>/scoring | grep '<title>'` (must be the Scoring title, not the homepage's). Preview deploys sit behind Vercel Deployment Protection, so curl needs an `x-vercel-protection-bypass` header — or use the browser's **View Source** (never the DevTools Elements panel, which shows the post-React DOM and would look correct either way).
...
### Ask Tutor (two surfaces)

The career-advisor chat ships on two surfaces sharing one hook, one avatar and one markdown renderer: the **turn-scoped** floating chat (below) and the **general coach** at `/tutor` (its own subsection at the end). Everything in this section is the turn chat unless it says otherwise.
...
- **Streaming seam:** `useApi().apiStream(path, init)` returns the raw `Response` (auth header + `ApiError` on non-2xx, but no `.json()`), and `lib/sse.ts:readSSE` parses the `event:`/`data:` frames. `useTutorChat({mode, sessionId?, turnId?})` is **mode-parameterized** — `mode` picks the endpoint (`/api/v1/sessions/{id}/turns/{turnId}/tutor` vs `/api/v1/tutor`), the request body, the credit cost and the greeting; everything downstream is identical, which is the point of sharing it. `send` POSTs and folds `tool`/`token`/`done`/`error` events into a discriminated-union message list (`{kind:'text'}` bubbles + `{kind:'tool'}` step chips). **History is text bubbles only, each clamped to 4000 chars** (matches the backend schema cap so a long reply can't 422 the next message); tool chips are excluded.
- **Composer guards (abuse caps; mirror the backend — see `../CLAUDE.md` → "Ask Tutor → Abuse caps").**
  - **Per-message length:** the textarea has `maxLength={MAX_MESSAGE_CHARS}` (`MAX_TURN_MESSAGE_CHARS` = 300, from `types/tutor.ts`; the general composer uses `MAX_GENERAL_MESSAGE_CHARS` = 1000) + a near-limit char counter (shown only when ≤40 left, amber at the cap); `submit` also slices defensively. The "Ask about this" `context_snippet` is separate and **not** counted toward the 300.
  - **Daily budget — CREDITS, not messages (20/day free):** the two surfaces draw on one budget at different rates, so `types/tutor.ts` exports `MAX_CHAT_CREDITS_PER_DAY` (20), `TURN_CHAT_CREDIT_COST` (1), `GENERAL_CHAT_CREDIT_COST` (2) and `chatCreditCost(mode)`. `useTutorChat` tracks `remaining: number | null` credits (null = Pro / unknown). **Seeded once from `useMe().me.daily_chat_count`** during render (React-19 "compare-to-tracked-state" pattern, not an effect, so a live decrement is never clobbered by a later `/me` broadcast), then driven live by the `done` event's `remaining` and set to `0` on a `429`. **`limitReached` compares `remaining < creditCost`, NOT `remaining === 0`** — with 1 credit left the turn chat still works and the general coach doesn't, which is the whole point of the weighting; `send` early-returns on the same condition. When unaffordable, the textarea **and** send button disable and a red `text-critique` "they reset at midnight" message shows (starters hidden), while the rest of the chat stays usable; when close, a muted `text-text-subtle` "N chat credits left today" hint names this surface's cost. Both cost numbers also appear on `/pricing` (`LIMITS`) and Settings → Pricing (`UsageRow`), interpolated from the same constants.
- **Markdown rendering — `TutorMarkdown.tsx`.** Tutor bubbles render through it; **user bubbles + tool labels stay raw** (`{...}` — users type plain text). It renders the tight subset the model is allowed to emit: `**bold**`, `*italic*`/`_italic_`, hyphen bullets (`<ul>`), numbered lists (`<ol>`). **Deliberately NOT a markdown library** — the subset is tiny and this stays XSS-safe by construction (only emits `<p>/<strong>/<em>/<ul>/<ol>/<li>/<a>` with React-escaped text; no `dangerouslySetInnerHTML` — load-bearing since replies echo untrusted transcript text and third-party search results). Anything outside the subset (a stray `#`/backtick) renders as **literal text**. The inline parser is **streaming-tolerant**: an unclosed `**`/`*`/`[` mid-stream falls back to its literal characters and resolves on the next token, so no broken markup flashes. Keep this in sync with the backend prompt's allowed-markdown rule (`tutor.py`'s `_FORMAT_TURN` / `_FORMAT_GENERAL`).
  - **Links are opt-in via `allowLinks`, and ONLY the general coach passes it** — that's the surface with web access and a citation obligation; the turn chat keeps its stricter no-link guarantee. When on, `[text](url)` and bare `https://` runs become anchors (`target="_blank" rel="noopener noreferrer nofollow"`, `text-link`). **Every href goes through `lib/safeHref` first: https ONLY**, which rejects `javascript:`/`data:`/`vbscript:`/`file:`, protocol-relative `//evil.example`, and plain `http:`; anything rejected renders as literal text rather than a link. That check is the second line of defense behind the prompt's reputable-domain rule — the model picks WHICH links to give, `safeHref` decides whether a string may be a link at all, so it must not depend on the model behaving. `safeHref` lives in `lib/` (not as an export of `TutorMarkdown.tsx`) because `react-refresh/only-export-components` requires component files to export only components.
...
- **Types** in `src/types/tutor.ts` mirror `backend/app/schemas/tutor.py` + the SSE events by hand (no codegen): `TutorMode`, `GeneralTutorMessageRequest`, `TutorDoneEvent.remaining?` (credits), the three credit constants (= backend `DAILY_CHAT_CREDITS_FREE` / `TURN_CHAT_CREDIT_COST` / `GENERAL_CHAT_CREDIT_COST`) and the two message caps; the spent-credit count comes from `MeResponse.daily_chat_count` in `src/types/user.ts`. On `ApiError`: **429 → set `remaining=0` and return (no bubble; the disabled/red-footer state speaks for it)**; 422 → soft tutor bubble using the server detail (moderation refusal / redirect / message too long); 503 → "content checks unavailable" bubble.

### Ask Tutor — the general coach (`/tutor`)

The turn chat can only ever discuss ONE answer, so the questions candidates have *between* sessions had nowhere to go. `/tutor` is that conversation: a full-page chat on a stronger model with live web search and cited sources (backend contract in `../CLAUDE.md` → "Ask Tutor"). Linked from the navbar; `RequireAuth` + `RequireOnboarded`, because the prompt is built from the candidate's experience level and target roles.

- **`pages/Tutor.tsx`** is a thin shell — TopBar + a heading + `<GeneralTutorChat />` — laid out `h-screen overflow-hidden` with the conversation column owning the **only** scroll, so the composer stays pinned the way a chat is expected to behave. That's also why there's no `SiteFooter` on this route.
- **`components/tutor/GeneralTutorChat.tsx`** is the conversation surface, and is **deliberately NOT a variant of `AskTutorChat`**: that component is mostly drag/resize/portal/bottom-sheet machinery for a floating window, none of which applies to a full page. What genuinely IS shared goes through modules — `useTutorChat` (reducer + SSE + quota), `PieMark` (avatar), `TutorMarkdown` (with `allowLinks`) — not through a `variant` prop.
- **Starters** live in `components/tutor/starters.ts` (own module for `react-refresh/only-export-components`) and map to the four jobs a single turn can't do: company homework, question-type craft, reading your own progress, building a story from your background. Tapping one **fills** the composer without sending, matching the turn chat's deliberate choice.
- **The conversation survives navigation.** `useTutorChat` parks the general list in a module-level `generalConversationStore`, so leaving `/tutor` for History and coming back returns to the same conversation; a hard refresh clears it, which keeps the backend's "persists NOTHING" contract intact. Because it persists across mounts there has to be a way to put it down, hence the **new-chat** control beside the send button (`reset`, disabled while streaming or already empty).
- **The credit balance is ALWAYS shown under the composer here**, not only near the limit like the turn chat's hint: a chat on this surface costs 2 of 20, so without the balance a user can't tell whether they have nine questions left or one. The line names the per-chat cost for the same reason — the number is only budgetable if you can divide by something.
- **Scroll-anchors on a sent message only, never on a streamed token** (ported from `AskTutorChat`): sending drops the new user bubble + typing dots to the bottom, and the reply then grows downward from the top of the column. Anchoring per token jumps the reader to the END of a reply they haven't read — worse here than in the turn chat, since replies run 7-8 sentences.
- **Time-to-first-token is long here** (high reasoning effort upstream), so the typing indicator carries more weight than it does in the turn chat — it keys off `isStreaming` and shows until the first token or tool chip lands.
...
| Amber action button   | `bg-highlight` + `text-primary-700` (or `<Button variant="amber">`) | amber `#FFA630` + dark ink (both themes; Practice _End recording_ + _Submit answer_) |
