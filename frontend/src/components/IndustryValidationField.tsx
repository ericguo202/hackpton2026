...
type Props = {
  ...
  onValidationChange?: (validation: IndustryValidation | null) => void;
...
export default function IndustryValidationField({
  value,
  onChange,
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        const result = await apiFetch<IndustryValidation>(
          `/api/v1/validation/industries?q=${encodeURIComponent(debounced)}`,
        );
        if (!cancelled) {
  ...
  function choose(alt: IndustryAlternative) {
    const accepted: IndustryValidation = {
      status: 'valid',
      user_input: alt.name,
      canonical_industry: alt.name,
    ...
    setAcceptedResult({ query: alt.name, data: accepted });
    setValidationResult({ query: alt.name, data: accepted });
    setErrorResult(null);
    onAcceptedSuggestion?.();
  }
  ...
  function acceptCustom() {
    const accepted: IndustryValidation = {
      ...
      canonical_industry: currentValue,
      category: visibleValidation?.category ?? null,
      source: 'custom',
      confidence: 0,
      alternatives: [],
  ...
  const helper =
    visibleValidation?.status === 'valid'
      ? `Matched to ${visibleValidation.canonical_industry}`
    ...
    <div className="space-y-2">
      ...
      {loadingPending && (
        <p className="flex items-center gap-2 text-xs leading-5 text-text-subtle">
          ...
          {showSlowConfirm ? 'Still checking industry...' : 'Checking industry...'}
        </p>
      )}
      {showSlowConfirm && (
        <button
        ...
        >
          Continue with this custom industry
        </button>
      )}
      {showAlternatives && (
      ...
      {needsConfirmation && onConfirmedCustomChange && (
        <label className="flex max-w-xl items-start gap-2 text-sm text-text-muted">
          ...
          />
          <span>Continue with this niche or custom industry.</span>
        </label>
      )}
      {invalid && (
        <p className="text-sm text-text-muted">Edit the industry to continue.</p>
      )}
    </div>
  );
}
