...
Flow (CLAUDE.md T+6-8, architecture steps 1 + 2):
  ...
  3. INSERT `interview_sessions` (status=in_progress) + turn 1 row in a
     single transaction; return `{session_id, summary, first_question}`.

Errors from either LLM call bubble up as 500s — acceptable for the
hackathon; the frontend shows a toast and the user retries.
"""
