"""
Situational opening-question prompts — the Situational (Hypothetical) question type.

Sibling of `_star_opening_prompts.py`, scoped to the **Situational** question
category. Unlike STAR ("tell me about a time…") these questions present a
realistic workplace SCENARIO and ask how the candidate WOULD handle it — and the
single load-bearing validity rule (from `situational_implementation.md`) is that
every question must contain a genuine **DILEMMA**: the scenario forces a choice
between two or more mutually exclusive courses of action, each with a real cost.
"What would you do if a teammate missed a deadline?" is weak; "your teammate
missed a deadline that blocks you, and flagging it will damage a relationship you
depend on — what do you do?" is a real situational question.

The scenario GENRE is field-specific (ethics dilemmas in healthcare, service
recovery in retail/sales, integrity/pressure in finance/legal/cyber, etc.), so
this module keys 15 per-field scenario themes + example pools off the shared
`FieldCategory` axis — exactly like `_star_opening_prompts.STAR_FIELD_THEMES` /
`STAR_FIELD_EXAMPLES`.

Deliberately **NOT experience-level tailored** at generation time (per the
implementation doc: situational question generation is industry-specific, not
level-specific — the level axis is evaluation-only via the "escalation flip" in
`_situational_evaluator_rubric`). So `build_situational_opening_prompt` takes only
the field category, with no experience-level PRIMARY-driver block (unlike the STAR
and Motivation & Fit builders).

Source-of-truth markdown: `backend/prompts/situational_opening_question_prompts.md`.
...
# Human-readable labels for the experience-level calibration note (a touch
# friendlier than the raw enum values, e.g. "entry-level" not "entry").
...
SITUATIONAL_OPENING_INTRO_TEMPLATE = """\
...
# Situational-specific form clause — the single hard constraint that defines the
# Situational shape AND encodes the dilemma requirement (the one validity-bearing
# design feature in the situational-interview literature). Kept separate from the
# question-form-neutral intro template exactly as `_STAR_OPENING_FORM_CLAUSE` is.
_SITUATIONAL_OPENING_FORM_CLAUSE = (
    "- A situational / hypothetical question — present a realistic workplace "
    "SCENARIO and ask how the candidate WOULD handle it. It MUST contain a "
    "genuine DILEMMA: the scenario forces a choice between two or more mutually "
    "exclusive courses of action, each with a real cost (for example, flagging a "
    "blocker versus protecting a relationship you depend on, or meeting a deadline "
    "versus raising a quality or safety concern). A vague 'what would you do if X "
    "happened?' with an obvious right answer is NOT acceptable — the tension "
    "between competing goods, stakeholders, or values is the entire point. It is "
    "NOT a 'tell me about a time…' past-behavior question and NOT a motivation/fit "
    "question."
)


# Light experience-level calibration (NOT a PRIMARY-driver matrix like STAR/M&F —
# situational question generation stays field-driven). It just keeps the scenario's
# scope, authority, and stakes realistic for the candidate's level so a field
# example that assumes seniority isn't posed to an intern. Empty-omission when the
# level is unknown (legacy).
_SITUATIONAL_LEVEL_NOTE_TEMPLATE = (
    "Experience-level fit — the candidate is at the {level} level. Keep the "
    "scenario's scope, authority, and stakes realistic for someone at this level: "
    "the competing choices must be ones a person at this level would plausibly "
    "face themselves. For example, do NOT ask an intern or entry-level candidate "
    "to manage, discipline, or fire a subordinate, or to make an executive-scope "
    "call; and do NOT hand a senior or executive candidate a trivially junior "
    "scenario. Adapt the dilemma (or lean toward a different field theme) so it "
    "fits this level."
)


# Field-keyed scenario THEMES — the breadth of dilemma genres each field tests
# (the dominant scenario genre for the field per the implementation doc's industry
# table). Shown in full so the model knows the bucket; the sampled examples steer
# the concrete shape.
SITUATIONAL_FIELD_SCENARIOS: dict[FieldCategory, list[str]] = {
...
# Field-keyed EXAMPLE dilemma questions. 2 sampled per call (the same 2-of-N
# anti-"fixed-attractor" rotation as `_star_opening_prompts.STAR_FIELD_EXAMPLES`).
# Every example poses a genuine dilemma (mutually exclusive courses of action).
SITUATIONAL_FIELD_EXAMPLES: dict[FieldCategory, list[str]] = {
    "Technology, Product, and Design": [
...
def build_situational_opening_prompt(
    category: FieldCategory,
    experience_level: ExperienceLevel | None = None,
    rng: random.Random | None = None,
) -> str:
    """Assemble the per-call system prompt for a Situational opening question.

    - Intro is the constant template with the category name substituted, followed
      by the situational form clause (`_SITUATIONAL_OPENING_FORM_CLAUSE`), which
      carries the load-bearing dilemma requirement.
    - When `experience_level` is known, a light calibration note is added so the
      scenario's scope/authority/stakes fit the candidate's level (e.g. no
      "manage a subordinate" dilemma for an intern). This is deliberately NOT a
      PRIMARY-driver experience matrix like STAR/M&F — situational generation
      stays field-driven; the level is just a fit guard. Omitted when None.
    - The full scenario-theme catalog for the field is shown so the model knows
      the breadth of dilemmas this field tests.
    - Two example dilemma questions are sampled at random per call (2-of-N
      rotation, mirroring `build_star_opening_prompt`) to break the "same question
      every time" attractor.

    `rng` is injectable so tests can pin the sample; production passes `None`.
    """
    themes = (
        SITUATIONAL_FIELD_SCENARIOS.get(category)
        or SITUATIONAL_FIELD_SCENARIOS[DEFAULT_CATEGORY]
    )
    examples = (
        SITUATIONAL_FIELD_EXAMPLES.get(category)
        or SITUATIONAL_FIELD_EXAMPLES[DEFAULT_CATEGORY]
    )
    sampler = rng if rng is not None else random
    sampled = sampler.sample(examples, 2) if len(examples) >= 2 else list(examples)

    intro = (
        SITUATIONAL_OPENING_INTRO_TEMPLATE.format(category=category)
        + "\n"
        + _SITUATIONAL_OPENING_FORM_CLAUSE
    )
    # Empty-omission: legacy (None) sessions keep the field-only prompt unchanged.
    level_section = ""
    if isinstance(experience_level, ExperienceLevel):
        level = _LEVEL_LABEL.get(experience_level, experience_level.value)
        level_section = _SITUATIONAL_LEVEL_NOTE_TEMPLATE.format(level=level) + "\n\n"
    themes_block = "\n".join(f"  - {t}" for t in themes)
    examples_block = "\n".join(f"  - {e}" for e in sampled)
    style_cues = (
        "Style cues (concrete dilemma shapes — emulate the spirit and the "
        "competing-choice tension, not the wording):\n"
        f"{examples_block}"
    )
    ...
    return (
        f"{intro}\n\n"
        f"{level_section}"
        "This field's situational questions probe dilemmas across these themes "
        "(use this as the breadth of what you can probe — do NOT limit yourself "
        f"to the two examples below):\n{themes_block}\n\n"
        f"{style_cues}"
    )
