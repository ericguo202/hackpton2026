...
- **Delivery score + structured delivery feedback (server-side, never LLM).** `evaluator.py:_compute_delivery_score(cv_summary)` derives the 0–10 `delivery` deterministically from MediaPipe webcam analytics: eye/posture calibrated bands + visual stability, minus coverage/streak penalties for severe sustained issues. The evaluator LLM is **not** given the webcam analytics block; `cv_summary` is used only after the LLM returns to compute deterministic delivery score/feedback and add a delivery quick-win. Hard caps clamp looking away (`looked_away_pct`), low face visibility (`face_visible_pct`), bad posture/head-tilt. **Facial energy is deliberately one soft channel:** normalized `expression_quality` contributes once to the calibrated-quality blend; `low_energy_pct` / its streak are coaching diagnostics only (never deductions/caps) so a calm speaker isn't penalized. Browser `low_energy` requires all three flat signals (`expression < 32`, `smile < 20`, mouth openness `< .028`). `_build_delivery_feedback` emits `delivery_feedback` — `summary` naming the biggest visible issue + optional `eye_contact` / `alignment` / `posture` / `expression` cues (each ≤270 chars, deterministic); `_delivery_quick_win` adds one `"Delivery: …"` quick-win. Omitted when there's no `cv_summary`.
  - **Face-only full-turn quality aggregation (v2).** `FrameSummary` no longer decays eye/expression/posture EMAs when MediaPipe temporarily loses the face — `face_visible_pct` owns that penalty so detector dropout is not counted twice. It also emits 10%-trimmed means from face-visible raw frame scores (`*_score_robust`) across the whole turn, preventing the final seconds or isolated landmark spikes from dominating. The backend blends these with the legacy EMA until `face_quality_sample_count` reaches 20 (about two phone seconds), then trusts the robust signal fully. Missing/v1 summaries stay on the legacy path. Score/percentage inputs are finite-clamped and streaks cannot exceed analyzed frames.
  - **Phone capture is tagged and calibrated separately.** `lib/captureMode.ts` selects `mobile_portrait` / `mobile_landscape` only for a coarse-pointer viewport under 900px. Portrait requests the front camera at an ideal 720×960 / 3:4, renders a bounded 3:4 `object-contain` preview/replay (no face-only 16:9 crop), and shows a prop-at-eye-level framing cue. `useFaceAnalyzer` samples phone video at 10fps (desktop 15fps) and appends `capture_mode` + available dimensions to `cv_summary`. The backend and `deliveryScoring.ts` mirror apply softer posture-stability/coverage weights and relaxed posture-only caps for tagged phone capture because camera motion is confounded with candidate motion; eye-contact and face-visibility deductions/caps are unchanged, and severe sustained phone alignment still caps the score. Missing/unknown modes keep the legacy desktop path.
